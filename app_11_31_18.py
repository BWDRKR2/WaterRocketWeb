...
def begin():
    ##r = requests.get('http://10.10.101.59:8000/launch')	
...
def arm():
    print "FUCK"
    ##r = requests.get('http://10.10.101.59:8000/arm')	
    ##status=r.status_code
...
def abort():
    ##r = requests.get('http://10.10.101.59:8000/abort')	
    ##status=r.status_code
    return render_template('abort.html')

@app.route('/fueled/<amount>', methods=['GET'])
def begin_fuel(amount):
    ##r = requests.get('http://10.10.101.59:8000/begin_fuel/'+ amount)	
    ##status=r.status_code
...
def end_fuel():
    ##r = requests.get('http://10.10.101.59:8000/end_fuel')	
    ##status=r.status_code
...
def unarm():
    ##r = requests.get('http://10.10.101.59:8000/unarm')	
    ##status=r.status_code
