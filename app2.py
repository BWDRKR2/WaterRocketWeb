...
def begin():
    r = requests.get('http://10.68.54.37:8000/launch')	
...
def begin_fuel(amount):
    r = requests.get('http://10.68.54.37:8000/begin_fuel/'+ amount)	
    status=r.status_code
...
@app.route('/fueling/<amount1>', methods=['GET'])
def end_fuel(amount1):
    timeout = "setTimeout(doSomething,1000); function doSomething() {location.href='http://108.74.158.146/fueled/" + amount1 + "'}"
    print timeout 
    return render_template('main.html', fueling=timeout, counter="<h4>Fueling       </h4>", arm="disabled", unarm="disabled", fuel="disabled", launch="disabled", abort="active")
